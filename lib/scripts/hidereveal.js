document.addEventListener('click', (e) => {
  if (e.target.matches(".hidebtn")) {
    ...
    let target = document.querySelector("#show" + btnId.slice(5));
    hideOrReveal(spanbtn,btnId,target);
  };
})

function hideOrReveal(spanbtn,btnId,target) {
  text1 = (btnId=="#hideIntro") ? "See more..." : (btnId.includes("hideCW")) ? "Show content warnings..." : "+";
  text2 = (btnId=="#hideIntro") ? "Hide"        : (btnId.includes("hideCW")) ? "Hide content warnings..." : "-";
